...
interface BluetoothLowEnergyApi {
  ...
  readCharacteristic: () => Promise<string | null>;
}
...
function useBLE(): BluetoothLowEnergyApi {
  ...
  const connectToDevice = async (device: Device) => {
    try {
      ...
      const result = await deviceConnection.discoverAllServicesAndCharacteristics();
  ...
  const findAudioCharacteristic = async (device: Device): Promise<Characteristic | null> => {
    try {
      ...
      for (const service of services) {
        ...
        const potentialAudioChar = characteristics.find((char) => char.isWritableWithoutResponse);
        if (potentialAudioChar) {
          console.log(`Potential audio characteristic found: ${potentialAudioChar.uuid}`);
  ...
  const adjustVolume = async (increase: boolean) => {
    ...
    try {
      console.log(`Attempting to adjust volume: ${increase ? 'increase' : 'decrease'}`);
      const isReady = await isDeviceReady();
      if (!isReady) {
        throw new Error('Device is not ready for interaction');
      }
      ...
      const audioChar = await findAudioCharacteristic(connectedDevice);
      if (audioChar) {
        console.log('Audio characteristic found, sending volume adjustment command');
  ...
  const readCharacteristic = async (): Promise<string | null> => {
    if (!connectedDevice) {
      console.log('No device connected');
      return null;
    }
    ...
    try {
      console.log('Attempting to read characteristic');
      const isReady = await isDeviceReady();
      if (!isReady) {
        throw new Error('Device is not ready for interaction');
      }
      ...
      const audioChar = await findAudioCharacteristic(connectedDevice);
      if (audioChar) {
        console.log('Audio characteristic found, reading value');
        const characteristicValue = await audioChar.read();
        const decodedValue = Buffer.from(characteristicValue.value, 'base64').toString('utf-8');
        console.log('Characteristic value read successfully:', decodedValue);
        return decodedValue;
      } else {
        throw new Error('Audio characteristic not found');
      }
